...
define('BOT_TOKEN', '242540296:AAFeXo4YjKh-TGNCRBWHiZ7LLiJ3O8oA4m8');
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "اگه تو هم تلگرامت آپدیت باشه این دکمه های شیشه ای رو دیدی صددرصد\n ما با همین دکمه ها براتون چند تا بازی آماده کردیم مثل ایکس او و سنگ کاغذ قیچی \n اگه مایلی با دوستات بازی کنی روی لینک زیر کلیک کن. بعد دوستتو انتخاب کن و منتظر باش که بااکس اینلاین ظاهر شه", 'reply_markup' => array(
            "inline_keyboard"=>array(
			    array(array("text"=>"توسعه دهنده","url"=>"https://telegram.me/cleverm")),
				array(array("text"=>"مشارکت کننده","url"=>"https://telegram.me/erf4n")),
...
function inlineMessage($inline){
	...
	apiRequest("answerInlineQuery",array("inline_query_id"=>$id,"results"=>array(array("type"=>"article","id"=>$query,"title"=>"بازی XO","input_message_content"=>array("message_text"=>"<b>کون دادن</b>\n برای شروع روی دکمه زیر کلیک کنید👇🏻👇🏻👇🏻","parse_mode"=>"HTML","disable_web_page_preview"=>false),
	    "reply_markup"=>array(
	        "inline_keyboard"=>array(
			    array(array("text"=>"شروع کون دادن!","callback_data"=>"play_".$chat_id))
			)
		)
	))));
	exit;
...
function callbackMessage($callback){
	  ...
	  if(strpos($data, "play") === 0){
		  $data=explode("_",$data);
		  if($data[1]==$pv_id){
			  apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"شما آغاز کننده ی این کون دادن هستید بنابراین باید یکی از کونی هاتون روی دکمه کلیک کنه!",'show_alert'=>false));
		      exit;
		  }
		  else{
		  ...
		  apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"کون دادن به اتمام رسید."," reply_markup"=>array(
			"inline_keyboard"=>$Tab 
		  )));  
		  exit;
	  }
		  ...
		  apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"کون دادن به پایان رسیده.","reply_markup"=>array(
			"inline_keyboard"=>$Tab 
		  )));  
		  exit;
	  }
		  ...
		  if($pv_id==$Turn){
			  ...
			  if($Tab[$i][$j]["text"]!=" ") apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"شما نمیتوانید دکمه مورد نظر را انتخاب کنید.",'show_alert'=>false));
                  ...
                  //				  
				    if(Win($Tab)=="⭕️"||Win($Tab)=="❌") {
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"کیری اول:$P1Name(❌)\nکیری دوم:$P2Name(⭕️)\n\nبرنده:".$winner."(".Win($Tab).")","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"کیری اول:$P1Name(❌)\nکیری دوم:$P2Name(⭕️)\n\nهیچکس کونشو نباخت","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
		    ...
		    apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"نوبتت نیست کیری.",'show_alert'=>false));
			exit;
		}
	}
	  //apiRequest("sendMessage",array("chat_id"=>111825543,"text"=>$data));
